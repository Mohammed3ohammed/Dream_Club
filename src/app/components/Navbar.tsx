  ...
  return (
    <nav className="z-50">
          <div className="bg-white opacity-95 h-20 flex justify-around items-center px-4 md:px-10 lg:px-20  z-50">
            <h1 className="text-orange-500 italic text-xl">Dream Club</h1>
            <div 
          ...
          onClick={() => setMenuOpen(!isMenuOpen)}
            ...
            <ul className={`${isMenuOpen ? "block" : "hidden"} absolute md:static top-20 left-0 w-full flex flex-col md:flex-row items-center md:w-auto bg-white md:bg-transparent md:flex text-black gap-5 lg:gap-10 p-4 md:p-0 z-50 text-right`}>
              <Link href="/"><li className="text-orange-500 py-1 md:py-0 mt-1 sm:mt-0 z-50">Home</li></Link>
              ...
              <Link href="/login" className="md:ml-10 border-2 border-solid border-orange-500 p-1 text-center w-32 rounded-full transition hover:bg-orange-500 hover:text-white">LogIn</Link>
              <Link href="/signup" className="border-2 border-solid border-orange-500 p-1 text-center w-32 rounded-full transition hover:bg-orange-500 hover:text-white">SignUp</Link>
