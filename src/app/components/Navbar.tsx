    ...
    <nav className="z-50">
          ...
          onClick={() => setMenuOpen(!isMenuOpen)}
            ...
            <ul className={`${isMenuOpen ? "block" : "hidden"} absolute md:static top-20 left-0 w-full flex flex-col md:flex-row items-center md:w-auto bg-white md:bg-transparent md:flex text-black gap-5 lg:gap-10 p-4 md:p-0 z-50 text-right`}>
              ...
              <Link href="/log" className="md:ml-10 border-2 border-solid border-orange-500 p-1 text-center w-32 rounded-full transition hover:bg-orange-500 hover:text-white">LogIn</Link>
              <Link href="/sign" className="border-2 border-solid border-orange-500 p-1 text-center w-32 rounded-full transition hover:bg-orange-500 hover:text-white">SignUp</Link>
              <Link href="/log/admin">
